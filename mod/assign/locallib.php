...
class assign
{
    ...
    public function view($action = '')
    {
        ...
        } else if ($action == 'gradingall') {
            $o .= $this->view_gradingall_page();
        ...
        } else if ($action == 'checkersubmission') {
            $o .= $this->view_checker_submission_page();
        } else if ($action == 'tester') {
            $o .= $this->view_tester_submission_page();
        } else if ($action == 'adminuploader') {
            $o .= $this->view_admin_uploader_page();
        } else {
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform)
    {
        ...
        //mycode
        $mform->addElement('checkbox', 'checkhomework', "Omogoči preverjanje nalog");
        $mform->disabledIf('checkhomework', 'assignsubmission_file_enabled');

        $default = get_config('checkhomework', 'default');
        $mform->setDefault('checkhomework', $default);
    ...
    protected function view_single_grade_page($mform)
    {
        ...
        require_once($CFG->dirroot . '/mod/assign/tester_form.php');
        ...
        $o .= $this->get_renderer()->heading("Rezultat testiranja", 3);
        $tester_form = new tester_form(null, array('userid' => $userid));
        $o .= $this->get_renderer()->render(new assign_form('tester_form', $tester_form));
    ...
    protected function view_grading_table()
    {
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
            ...
            $gradebookurl = "http://localhost/moodle/mod/assign/view.php?id=$_GET[id]&action=gradingall";
            $links[$gradebookurl] = "Testiraj vse";
        }
    ...
    protected function view_gradingall_page()
    {
        global $CFG;
        ...
        // Need submit permission to submit an assignment.
//        $this->require_view_grades();
        require_once($CFG->dirroot . '/mod/assign/custom_grade_form.php');
...
//        $o .= $this->view_grading_table();
        $o .= $this->get_renderer()->render(new assign_header($this->get_instance(),
            ...
            false,
            $this->get_course_module()->id,
            "Ocene vseh šutdentov"));

        $mform = new custom_grade_form(null);

        $o .= $this->get_renderer()->render(new assign_form('custom_form', $mform));
...
//             We only show this if it their submission.
    ...
    protected function view_checker_submission_page()
    {
        global $CFG, $USER, $DB;

        $o = '';
        require_once($CFG->dirroot . '/mod/assign/upload_form.php');
...
//        require_once($CFG->dirroot . '/mod/assign/submission_form.php');
        ...
        if ($userid == $USER->id) {
            ...
            $name = $this->fullname($user);
            $title = "Neki za preverjanje" + $name;
        }
        ...
        $o .= $this->get_renderer()->render(new assign_form('upload', $mform));
        $o .= $this->view_footer();

        return $o;
    }

    protected function view_tester_submission_page()
    {
        global $CFG, $USER, $DB;
...
//        require_once($CFG->dirroot . '/mod/assign/submission_form.php');
        ...
        $mform = new tester_form(null);

        $o .= $this->get_renderer()->render(new assign_form('tester', $mform));
        $o .= $this->view_footer();
    ...
    protected function view_admin_uploader_page()
    {
        global $CFG, $USER, $DB;

        $o = '';
        require_once($CFG->dirroot . '/mod/assign/admin_upload_form.php');
...
//        }
        ...
        $mform = new admin_upload_form();

        $o .= $this->get_renderer()->render(new assign_form('admin_upload', $mform));
        $o .= $this->view_footer();
...
//        $o .= $this->get_renderer()->render();
    ...
    protected function view_submission_page()
    {
        ...
        if ($this->can_view_submission($USER->id)) {
            //TODO:: Add button to check tests
            $o .= $this->view_student_summary($USER, true);
        }

        //teacher
        if (has_capability('moodle/course:update', $this->context) and $instance->checkhomework == 1) {
            $checkerurlparams = array('id' => $_GET['id'], 'action' => 'adminuploader');
            $o .= $this->output->single_button(new moodle_url('/mod/assign/view.php', $checkerurlparams),
                "Dodaj rešitve", 'get');
        }
    ...
    public function test_submission(stdClass $data, & $notices)
    {
        $mform = new tester_form($data);
