            ...
            if (!$status->teamsubmission || $status->submissiongroup != false || !$status->preventsubmissionnotingroup) {
                foreach ($status->submissionplugins as $plugin) {
                    ...
                    if ($plugin->is_enabled() &&
                        ...
                        $is_checkhw_enable = $GLOBALS['DB']->get_record('assign',array('id' => $submission->assignment))->checkhomework;
//                            $plugin->is_checking_in_assign_enable($submission->assignment);
                        $row = new html_table_row();
                        $cell1 = new html_table_cell($plugin->get_name());
        ...
        if ($status->view == assign_submission_status::STUDENT_VIEW) {
            if ($status->canedit) {
//                $is_checkhw_enable = $plugin->assignment->instance['checkhomework'];
                if (!$submission || $submission->status == ASSIGN_SUBMISSION_STATUS_NEW) {
                    ...
                    if ($is_checkhw_enable == 1) {
                        $checkerurlparams = array('id' => $status->coursemoduleid, 'action' => 'checkersubmission');
